    ...
    for symbol in ticker_list:
        try:
            ...
            result.append({
                ...
                "details": {
                    "open": info.get("regularMarketOpen") or info.get("open") or (hist["Open"].iloc[-1] if not hist.empty else None),
                    "dayHigh": info.get("regularMarketDayHigh") or info.get("dayHigh") or (hist["High"].iloc[-1] if not hist.empty else None),
                    "dayLow": info.get("regularMarketDayLow") or info.get("dayLow") or (hist["Low"].iloc[-1] if not hist.empty else None),
                    "volume": info.get("regularMarketVolume") or info.get("volume") or (int(hist["Volume"].iloc[-1]) if not hist.empty else None),
